...
from database.queries import (
    ...
    get_customer_by_id, get_signature_by_customer_id, get_signature_by_plan_id,
    create_signature, get_signature_by_id, get_company_task_by_id, create_company_task
)
...
def test_create_company(db_session):
    ...
    new_company = create_company(db_session, "Test Company", "test_tenant", "logo.png")
...
def test_get_signature_by_plan_id(db_session):
    db_session.query().filter().first.return_value = Signature(id="1")
    signature = get_signature_by_plan_id(db_session, "1")
    assert signature.id == "1"
